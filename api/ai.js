...
export default async function handler(req, res) {
  ...
  const configuredModel = (process.env.AI_MODEL || 'gpt-4o-mini').trim().replace(/^models\//, '').replace(/^gemini-(?:2\.0|3\.6)-flash(?:-.*)?$/i, 'gemini-2.5-flash-lite')
  ...
  const base = isGemini
    ...
    : configuredBase
  const model = isGemini && !configuredModel.toLowerCase().includes('gemini') ? 'gemini-2.5-flash-lite' : configuredModel
  const url = isGemini
